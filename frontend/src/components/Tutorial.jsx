...
const GENERATING_STEPS = [
  {
    target: ".building-stages",
    position: "right",
    title: "K2-Think V3 is reasoning",
    body: "Each stage shows elapsed time. On completed stages, click ▼ Reasoning to read K2-Think V3's exact thought process — the design decisions it made for your specific prompt.",
    k2: "K2-Think V3 reasons holistically: it plans the component layout, picks section types, designs sample data, and outputs the full JSON schema in a single reasoning pass.",
    hasNext: false,
    waitForNav: true,
  },
];
...
function getBubbleStyle(targetEl, position, narrow = false) {
  ...
  const gap = 14;
  const bubbleW = isMobile ? vw - gap * 2 : (narrow ? 220 : 340);
  const bubbleH = 300;
  ...
  if (isMobile) {
    ...
    const minRoom = 120;
    if (spaceBelow < minRoom && spaceAbove < minRoom) {
      return { position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: bubbleW, maxWidth: "none" };
    }
    const top = spaceBelow >= spaceAbove ? rect.bottom + gap : rect.top - bubbleH - gap;
    return { position: "fixed", top: Math.max(gap, Math.min(top, vh - bubbleH - gap)), left: gap, width: bubbleW, maxWidth: "none" };
  }
...
export default function Tutorial({ phase, onExit }) {
  ...
  const steps =
    ...
    phase === "generating" ? GENERATING_STEPS :
    phase === "ready"      ? READY_STEPS : [];

  const step = steps[stepIdx] || null;

  const updatePos = useCallback(() => {
    if (!step || !targetEl) { setBubblePos(null); setSpotlightPos(null); return; }
    setBubblePos(getBubbleStyle(targetEl, step.position, step.narrow));
  ...
  // Elevate target so it's above the overlay
  useEffect(() => {
    if (!targetEl) return;
  ...
  const waitHint =
    phase === "generating" ? "Waiting for prototype to generate…" :
    phase === "idle"       ? "Click an example to continue" : null;
  ...
  const bubble = bubblePos && (
    ...
    >
      ...
      <div className="tutorial-bubble-actions">
        {step.hasNext && (
          <button className="tutorial-next-btn" onClick={handleNext}>Next →</button>
        )}
        {step.allowInteract && !step.waitForNav && (
          <button className="tutorial-next-btn" onClick={handleNext}>Next →</button>
        )}
        {step.waitForNav && (
          <span className="tutorial-hint">{waitHint}</span>
        )}
        {step.isFinal && (
          <button className="tutorial-next-btn" onClick={onExit}>Finish tour ✓</button>
        )}
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      {createPortal(
        <button className="tutorial-exit-btn" style={{ zIndex: 9030 }} onClick={onExit}>
          ✕ Exit tour
        </button>,
        document.body
      )}
